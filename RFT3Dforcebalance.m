...
%% run all depth conditions

if run_forceBalance == 1
    h = waitbar(0,'Initializing waitbar...');
    tic
    runDepth(all_results, pointList, normalList, areaList, vcenter, radius, sf, h);
    toc
end
...
function runDepth(all_results, pointList, normalList, areaList, vcenter, radius, sf, h)
bList = [0 15 30 45 60 75 90];
wlist = [0.00001 3 5 8 10 12 20 40 100];
...
for i = 1:7
    ...
    for j = 1:9
        ...
        for k = 1: 1000
            ...
            Ftractive = Force(2);
            Fload = Force(3);
            Depth(j, i, k) = struct('ForceX', Ftractive, ...
                'ForceY', Fsidewall , ...
                'ForceZ', Fload, ...
                'wr', wr, ...
                'depth', sinkage, ...
    ...
    waitbar(i/7, h, 'In progress...')
end
waitbar(1,h,'Completed.');
disp("Done.");
close(h);
save('output/forceBalance.mat','Depth');
