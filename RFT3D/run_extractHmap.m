...
%     s1.EdgeColor = 'none';
    hold on
    scatter3(wheelPos(1), wheelPos(2), wheelPos(3),'r', 'filled')
    legend('Sand Heighmap', 'Wheel location')
    view(-105,45)
    daspect([1 1 1])
    hold off 
    axis equal
    ...
    figure
    hold on 
    p = plot(f1, [x y], z, 'Exclude', exclude);
    view(-105,45)
    alpha(.5)
...
%     s.EdgeColor = 'none';
    hold off
    daspect([1 1 1])
    legend('Included Points','Excluded Points', 'Fitted Curve');
%     plot(f1, 'FaceAlpha', 0.5);
